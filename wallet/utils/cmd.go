...
import (
	"flag"
	"fmt"
	"github.com/ethereum/go-ethereum/wallet/accounts"
	"github.com/ethereum/go-ethereum/wallet/accounts/keystore"
	"github.com/ethereum/go-ethereum/wallet/common"
	local "github.com/ethereum/go-ethereum/wallet/core/types"
	web3cmn "github.com/ethereum/go-ethereum/web3go/common"
	transaction "github.com/ethereum/go-ethereum/contracts/types"
	"github.com/ethereum/go-ethereum/web3go/provider"
	"github.com/ethereum/go-ethereum/web3go/rpc"
	"github.com/ethereum/go-ethereum/web3go/web3"
...
func SignTx(address string, tx *transaction.Transaction, chainID *big.Int) (*transaction.Transaction, error) {
	keyStoreDir := keystore.KeyStoreScheme
	return SignTxByDir(address, tx, chainID, keyStoreDir, nil)
}

func SignTxByPassWord(tx *transaction.Transaction, password string) (*transaction.Transaction, error) {
	keyStoreDir := keystore.KeyStoreScheme
...
func SignTxByDir(address string, tx *transaction.Transaction, chainID *big.Int, keystoreDir string, password *string) (*transaction.Transaction, error) {
	scryptN, scryptP, keydir, err := AccountConfig(keystoreDir)
	if err != nil {
...
func SendTransaction(tx *transaction.Transaction) (common.Hash, error) {
	//format 0x string
...
func SendRawTransaction(tx *transaction.Transaction) (common.Hash, error) {
	hostname := flag.String("hostname", "127.0.0.1", "The ethereum client RPC host")
	port := flag.String("port", "47768", "The ethereum client RPC port")
...
func SendRawTransactionWeb3(web *web3.Web3, txBytesStr string) (common.Hash, error) {
	...
	bytes := common.FromHex(txBytesStr)
