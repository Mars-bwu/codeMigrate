...
import (
	"errors"
	"github.com/ethereum/go-ethereum/core/vm/util"
	"math/big"
...
//不直接调用而是多包一层的原因是因为   1.gas以及要知道    2是哪个合约么
//因为我们的合约不用判断是哪个所以不用这不，但是如果因为gas可能就要了，但是不要也没事，保持一致，但是gas传过去就不要好了
func sysContractCall(evm *EVM, caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	//                **            ***                               **args对应这个
	sysContractExecutionFunc := GetSystemContractExecFunc(addr)
	//判断调用的是哪个合约
	//返回合约方法---最后会返回一个方法
	//会被填入系统合约
	//调用GetSystemContractExecFunc方法     该方法后面是根据一个映射得到一个类型为函数类型的东西（一个方法）

	ret, err = sysContractExecutionFunc(evm, caller, input)//这里干了什么调用了方法么
	//因为gas没有所以多包了一层么
	return ret, gas, err//这就是执行完合约以后像不调用外部合约一样获得的返回值
}